...
const DESTINATIONS = [
  ...
  },
  {
    ...
    subtitle: 'Where Luxury Meets the Desert',
    body: 'Experience the city of gold — from towering skyscrapers to ancient souks. Dubai is the ultimate layover destination upgraded to a full adventure.',
  ...
  },
  {
    ...
    region: 'Europe' as Region,
    subtitle: 'The Gateway to Europe',
    body: 'Navigate London like a local — from the Tube to hidden Borough markets, our guide helps Nigerian travelers make the most of their UK stopover or extended stay.',
    tags: ['Urban', 'Culture', 'History'],
  ...
  },
  {
    ...
    subtitle: 'The Jewel of the Caribbean Coast',
    body: "Step into a living museum. Cartagena's colonial walls, vibrant plazas, and warm Caribbean beaches make it the perfect gateway to South America.",
  ...
  },
  {
    ...
    body: 'Tango, steak, and European grandeur in the heart of the Southern Cone. Buenos Aires rewards the curious traveler with endless culture and passion.',
    tags: ['Food', 'Dance', 'Architecture'],
  ...
  },
  {
    ...
    country: 'Netherlands',
    region: 'Europe' as Region,
    subtitle: 'Canals, Culture, and Openness',
    body: "Cycle through 17th-century canal rings, visit world-class museums, and experience one of Europe's most welcoming cities for first-time visitors.",
    tags: ['Art', 'Culture', 'Architecture'],
    gradient: 'linear-gradient(135deg, #dc2626 0%, #991b1b 100%)',
  },
];

const REGIONS: Region[] = ['All', 'Africa', 'Europe', 'Middle East', 'Americas'];
...
export default function DestinationsPage() {
  ...
  return (
    <div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 pt-36 pb-20 w-full">
          ...
          >
            ...
            <p className="text-xl text-white/45 max-w-xl mx-auto mb-10" style={{ fontFamily: 'Satoshi, sans-serif' }}>
              Travel like a local, no matter where you land.
            </p>
      ...
      {/* ── SECTION 2: FEATURED DESTINATION (Serengeti) ── */}
      <section className="py-16 px-6 bg-[#f5f8fc] dark:bg-background">
                ...
                <div className="relative z-10 h-full flex flex-col justify-end p-10">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="mt-2 text-white/70 text-lg" style={{ fontFamily: 'Satoshi, sans-serif' }}>The Ultimate African Wildlife Experience</p>
                    </div>
      ...
      <section className="pb-28 px-6 bg-[#f5f8fc] dark:bg-background">
        <div className="max-w-7xl mx-auto">
          {/* Region filter tabs */}
            ...
            {REGIONS.map((r) => (
              <button
                ...
                style={{
                  ...
                  background: region === r ? '#1a2f5a' : 'transparent',
                  color: region === r ? '#fff' : 'inherit',
                  border: region === r ? '1px solid transparent' : '1px solid rgba(26,47,90,0.2)',
                }}
              >
                {r}
              </button>
              ...
              {filtered.map((dest) => (
                ...
                >
                  <Link to="/destinations" className="group block h-full">
                    <div className="h-full rounded-2xl overflow-hidden border border-slate-100 dark:border-border hover:border-blue-200 dark:hover:border-blue-900 hover:shadow-xl dark:hover:shadow-black/30 transition-all duration-500 hover:-translate-y-1 bg-white dark:bg-card">
                      <div className="h-40 relative" style={{ background: dest.gradient }}>
                        <div className="absolute inset-0 bg-black/20" />
                        <div className="absolute top-4 right-4">
                          <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-white/15 text-white backdrop-blur-sm" style={{ fontFamily: 'Satoshi, sans-serif' }}>
                            {dest.region}
                          </span>
                        </div>
                      ...
                      <div className="p-6">
                        <p className="text-blue-600 dark:text-blue-400 text-xs font-bold mb-2" style={{ fontFamily: 'Satoshi, sans-serif' }}>{dest.subtitle}</p>
                        <p className="text-sm text-slate-500 dark:text-muted-foreground leading-relaxed mb-4" style={{ fontFamily: 'Satoshi, sans-serif' }}>{dest.body}</p>
                        <div className="flex flex-wrap gap-1.5">
                          {dest.tags.map((tag) => (
                            <span key={tag} className="px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-slate-100 dark:bg-muted text-slate-500 dark:text-muted-foreground" style={{ fontFamily: 'Satoshi, sans-serif' }}>
                              {tag}
                            </span>
                          ))}
                        </div>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-20">
              <p className="text-slate-400 dark:text-muted-foreground text-lg" style={{ fontFamily: 'Satoshi, sans-serif' }}>
      ...
      {/* ── CTA BANNER ── */}
      <CTABanner heading1="Ready to Book Your" heading2="Next" headingAccent="Destination?" sub="Tell us where you want to go and we'll build the perfect package." />
    </div>
  );
}
